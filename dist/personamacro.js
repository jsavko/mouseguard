...
function main() {
  ...
  let currentHeroPoints = checkHeroPersona();
  for (let i = 0; i < currentHeroPoints.length; i++) {
    currentHeroPointsList += '<li>' + currentHeroPoints[i][0] + ' - Persona:' + currentHeroPoints[i][1]+ '</li>';    
  }  
...
function checkHeroPersona() {
    ...
    characters.forEach( (c) => {
      ...
      heros.push([c.data.name, c.data.data.rewards.persona]);
    }); 
    return heros;
  }

  async function Personamanager(html){
  ...
  async function updateHeroPoints(playerName, heroPoints) {
    ...
    let currentHeroPoints = parseInt( character.data.data.rewards.persona);
    let total = currentHeroPoints + parseInt( heroPoints );
    await character.update({['data.rewards.persona']: total});
    PersonaMessage(character, heroPoints);
  }
  ...
  function updateAllHerosXP(heroPoints) {
    ...
    players.map(async player => {    
      let currentHeroPoints = parseInt( player.data.data.rewards.persona);
      let total = currentHeroPoints + parseInt( heroPoints );
      await player.update({['data.rewards.persona']: total});
      PersonaMessage(player, heroPoints);  
    });
  }


  function PersonaMessage(player, points) {
    let message = `<h2>${player.data.name}</h2>`;  
    message += `<p>Received <b>${points}</b> Persona.</p>`;  
